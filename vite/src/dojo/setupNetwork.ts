...
export async function setupNetwork() {
  ...
  if (!import.meta.env.VITE_PUBLIC_WORLD_ADDRESS) throw (`NEXT_PUBLIC_WORLD_ADDRESS is null`)
  if (!import.meta.env.VITE_PUBLIC_NODE_URL) throw (`NEXT_PUBLIC_NODE_URL is null`)
  if (!import.meta.env.VITE_PUBLIC_TORII) throw (`NEXT_PUBLIC_TORII is null`)

  // Create a new DojoProvider instance.
  const provider = new DojoProvider(manifest, import.meta.env.VITE_PUBLIC_NODE_URL)
